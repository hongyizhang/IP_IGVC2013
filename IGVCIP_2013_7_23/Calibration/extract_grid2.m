function [x,X,n_sq_x,n_sq_y,ind_orig,ind_x,ind_y] = extract_grid2(I,wintx,winty,fc,cc,kc,dX,dY,xr,yr,click_mode);
%global 
if nargin < 11,
    click_mode = 0;
end;
...
prompt = {'Enter Size dX of each square along the X direction ([]=30mm) =','Enter Size dY of each square along the Y direction ([]=30mm) ='};
    %prompt = {'Enter x_axis offset(m):','Enter y_axis offset:(m)'};
    dlg_title = 'Input Size dX of each square along the X direction ([]=30mm) =, Enter Size dY of each square along the Y direction ([]=30mm) =';
    num_lines = 1;
    def = {'1000','1000'};
    %def = {'0.57','-0.23'};
    options.Resize='on';
    answer = inputdlg(prompt,dlg_title,num_lines,def,options);
%     dX = 1000   %mm  stagehusky size of 1 square
%     dY = 1000   %mm  stagehusky size of 1 square
    dX=str2num(answer{1});
    dY=str2num(answer{2});
...
%     dX=[];
%     if isempty(dX), dX = 97.25; end;
%     %if isempty(dY), dY = 30; end;
%     dY=[];
%     if isempty(dY), dY = 97.25; end;
